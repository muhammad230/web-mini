...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .sidebar { width: 240px; min-height: 100vh; background: #16302A; display: flex; flex-direction: column; position: fixed; top: 0; left: 0; z-index: 100; transition: width 0.2s; }
        ...
        .sidebar-bottom { padding: 16px; border-top: 1px solid #1e4238; }
...
<div class="main" id="main">
    ...
    <div class="content">
        <div class="page-title">Jobs / Bookings</div>
        <div class="page-sub">Manage all jobs on the platform</div>
        <div class="table-card">
            <div class="table-header">
                <div>
                    <div class="card-title" style="font-size:1rem;font-weight:700;color:#111827;">All Jobs</div>
                </div>
                <form method="GET" action="{{ route('admin.jobs') }}" class="filters">
                    <select name="status" class="filter-input">
                        <option value="">All Statuses</option>
                        <option value="pending_match" {{ request('status') == 'pending_match' ? 'selected' : '' }}>Pending Match</option>
                        <option value="quotes_received" {{ request('status') == 'quotes_received' ? 'selected' : '' }}>Quotes Received</option>
                        <option value="scheduled" {{ request('status') == 'scheduled' ? 'selected' : '' }}>Scheduled</option>
                        <option value="in_progress" {{ request('status') == 'in_progress' ? 'selected' : '' }}>In Progress</option>
                        <option value="completed" {{ request('status') == 'completed' ? 'selected' : '' }}>Completed</option>
                        <option value="cancelled" {{ request('status') == 'cancelled' ? 'selected' : '' }}>Cancelled</option>
                    </select>
                    <select name="trade" class="filter-input">
                        <option value="">All Trades</option>
                        <option value="Plumbing" {{ request('trade') == 'Plumbing' ? 'selected' : '' }}>Plumbing</option>
                        <option value="Electrical" {{ request('trade') == 'Electrical' ? 'selected' : '' }}>Electrical</option>
                        <option value="Carpentry" {{ request('trade') == 'Carpentry' ? 'selected' : '' }}>Carpentry</option>
                        <option value="Painting" {{ request('trade') == 'Painting' ? 'selected' : '' }}>Painting</option>
                    </select>
                    <input type="date" name="date_from" class="filter-input" placeholder="From Date" value="{{ request('date_from') }}">
                    <input type="date" name="date_to" class="filter-input" placeholder="To Date" value="{{ request('date_to') }}">
            ...
            <table>
                ...
                <tbody>
                    @if($jobs->count() > 0)
                        ...
                        @endforeach
                    @else
                        <tr><td colspan="8" style="text-align:center;padding:30px;color:#9ca3af;">No jobs found</td></tr>
                    @endif
                </tbody>
            </table>
            @if($jobs->hasPages())
